...
from models.networks.restormer import CrossTransformerBlock
try:
    import apex
...
class SPADE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, PONO=False, use_apex=False):
        ...
        if self.pad_type != 'zero':
            ...
            self.mlp_shared_img = nn.Sequential(
                nn.ReflectionPad2d(pw),
                nn.Conv2d(256, nhidden, kernel_size=ks, padding=0),
                nn.ReLU()
            )
    ...
    # def forward(self, x, segmap, similarity_map=None):
        ...
        # return adain(x, gamma)
        self.cross_trans = CrossTransformerBlock(dim=nhidden, num_heads=2, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='BiasFree')
    def forward(self, x, segmap,ref_img, similarity_map=None):
        ...
        segmap = F.interpolate(segmap, size=x.size()[2:], mode='nearest')
        ref_img = F.interpolate(ref_img, size=x.size()[2:], mode='nearest')
        actv = self.mlp_shared(segmap)
        #ref_actv = self.mlp_shared_img(ref_img)
        actv = self.cross_trans(actv,ref_img)
        if self.pad_type != 'zero':
            gamma = self.mlp_gamma(self.pad(actv))
