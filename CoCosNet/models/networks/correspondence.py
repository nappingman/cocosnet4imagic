...
class NoVGGCorrespondence(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        label_nc = 3 if opt.maskmix else 0
        ...
        self.beta = nn.Conv2d(in_channels=self.in_channels + label_nc + coord_c, out_channels=128, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self,
        ...
        channel_ref = ref_features.shape[1]
        ref_features = ref_features.view(batch_size, channel_ref, -1)
        ref_features = ref_features.permute(0, 2, 1)

        y = torch.matmul(f_div_C, ref)  # 2*1936*channel
        
        warped_ref_features = torch.matmul(f_div_C, ref_features)  # 2*1936*channel
        if self.opt.warp_patch:
            y = y.permute(0, 2, 1)
        ...
        warped_ref_features = warped_ref_features.permute(0, 2, 1).contiguous()
        warped_ref_features = warped_ref_features.view(batch_size, channel_ref, feature_height, feature_width)  # 2*3*44*44
        ...
        coor_out['warp_ref_feature'] = self.beta(warped_ref_features) if self.opt.warp_patch else self.upsampling(self.beta(warped_ref_features))
