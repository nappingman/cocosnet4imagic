...
from models.networks.normalization_v1 import SPADE, equal_lr, SPADE_TwoPath
...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt, use_se=False, dilation=1):
        ...
        if 'spade_ic' in opt:
            ...
            ic = 0 + (3 if 'warp' in opt.CBN_intype else 0) + (3 if 'mask' in opt.CBN_intype else 0)
    ...
    def forward(self, x, seg1,ref_img):
        x_s = self.shortcut(x, seg1,ref_img)
        if self.pad_type != 'zero':
            dx = self.conv_0(self.pad(self.actvn(self.norm_0(x, seg1,ref_img))))
            dx = self.conv_1(self.pad(self.actvn(self.norm_1(dx, seg1,ref_img))))
            if self.use_se:
                dx = self.se_layar(dx)
        else:
            dx = self.conv_0(self.actvn(self.norm_0(x, seg1,ref_img)))
            dx = self.conv_1(self.actvn(self.norm_1(dx, seg1,ref_img)))
            if self.use_se:
                dx = self.se_layar(dx)
    ...
    def shortcut(self, x, seg1,ref_img):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, seg1,ref_img))
        else:
            x_s = x
        return x_s
