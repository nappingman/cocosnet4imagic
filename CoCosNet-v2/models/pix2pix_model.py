...
class Pix2PixModel(torch.nn.Module):
    ...
    def forward(self, data, mode, GforD=None):
        ...
        if mode == 'generator':
            ...
            with torch.no_grad():
                out = self.inference(input_semantics, ref_semantics=ref_semantics[rand_idx], \
                                        ref_image=ref_image[rand_idx], self_ref=self_ref, \
    ...
    def compute_generator_loss(self, input_label, input_semantics, real_image, ref_label=None, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        if self.opt.weight_warp_self > 0:
            ...
            G_losses['G_warp_self'] = \
                torch.mean(F.l1_loss(warp4, real_image, reduction='none') ) * self.opt.weight_warp_self * 1.0 + \
                torch.mean(F.l1_loss(warp3, F.avg_pool2d(real_image, 2, stride=2), reduction='none')) * self.opt.weight_warp_self * 1.0 + \
                torch.mean(F.l1_loss(warp2, F.avg_pool2d(real_image, 4, stride=4), reduction='none')) * self.opt.weight_warp_self * 1.0 + \
                torch.mean(F.l1_loss(warp1, F.avg_pool2d(real_image, 8, stride=8), reduction='none')) * self.opt.weight_warp_self * 1.0
        """gan loss"""
        ...
        for i in range(len(generate_out['real_features'])):
            loss += weights[i] * self.criterionFeat(fake_features[i], generate_out['real_features'][i].detach())
