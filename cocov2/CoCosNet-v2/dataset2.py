...
class IllustTestDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.valid_size =valid_size
        self.data_path = data_path
        self.pathlist = sorted(list(self.data_path.glob(f"**/*{extension}")))
        self.sketchlist = sorted(list(sketch_path.glob(f"**/*{extension}")))
    ...
    def __getitem__(self, idx):
        ...
        warped_line = (warped_line * 255.0).reshape(warped_line.shape[0], warped_line.shape[1], 1)
        warped_line = np.tile(warped_line, (1, 1, 3))
